...
class _BudgetsPageState extends State<BudgetsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final rows = await ApiClient.instance.budgets(search: _search.text, state: _state, status: _status, company: _company);
  ...
  Future<void> _filters() async {
    ...
    await showModalBottomSheet(context: context, isScrollControlled: true, builder: (context) => StatefulBuilder(builder: (context, setSheet) => Padding(
      ...
      child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [Expanded(child: OutlinedButton(onPressed: () { state=''; status=''; company=''; setSheet((){}); }, child: const Text('Limpiar'))), const SizedBox(width: 8), Expanded(child: FilledButton(onPressed: () { _state=state; _status=status; _company=company; Navigator.pop(context); _load(); }, child: const Text('Aplicar')))]),
      ]),
    )));
  }
  ...
  Widget build(BuildContext context) {
    final pendingCount = _rows.where(_isPendingBudget).length;
    ...
    child: ListView(padding: const EdgeInsets.fromLTRB(16, 18, 16, 100), children: [
      ...
      if (!_loading && _error == null && pendingCount > 0) ...[
        Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Text('$pendingCount pendiente${pendingCount == 1 ? '' : 's'} de aprobación', style: const TextStyle(color: DalvoColors.warning, fontWeight: FontWeight.w900))),
          ]),
        ),
      ...
      if (_loading) const Center(child: Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator()))
        ...
        await Navigator.of(context).push(MaterialPageRoute(builder: (_) => BudgetDetailPage(budgetId: int.tryParse('${row['id']}') ?? 0)));
        _load();
      }))),
    ]),
  );
  ...
  bool _isPendingBudget(Map<String, dynamic> row) {
...
class _BudgetDetailPageState extends State<BudgetDetailPage> {
  ...
  Future<void> _status(String action) async {
    ...
    final confirm=await showDialog<bool>(context: context,builder:(context)=>AlertDialog(title:Text(approve?'Aprobar presupuesto':'No aprobar presupuesto'),content:Text(approve?'Se marcará el presupuesto como aprobado.':'Se marcará el presupuesto como no aprobado.'),actions:[TextButton(onPressed:()=>Navigator.pop(context,false),child:const Text('Cancelar')),FilledButton(onPressed:()=>Navigator.pop(context,true),child:Text(approve?'Aprobar':'No aprobar'))]));
    if(confirm!=true)return; setState(()=>_busy=true);
    try{await ApiClient.instance.updateBudgetStatus(widget.budgetId,action);await _load();}catch(error){if(mounted)ScaffoldMessenger.of(context).showSnackBar(SnackBar(content:Text(error is ApiException?error.message:'No se pudo actualizar.')));}finally{if(mounted)setState(()=>_busy=false);}
  }
  @override
  Widget build(BuildContext context){
    ...
    return Scaffold(appBar:AppBar(title:Text(budget?['folio']?.toString()??'Presupuesto')),body:_error!=null?Center(child:DalvoEmptyState(icon:Icons.error_outline,title:'No se pudo cargar',message:_error!,error:true,action:OutlinedButton(onPressed:_load,child:const Text('Reintentar')))):data==null?const Center(child:CircularProgressIndicator()):RefreshIndicator(onRefresh:_load,child:ListView(padding:const EdgeInsets.fromLTRB(16,10,16,30),children:[
      ...
      const SizedBox(height:16),DalvoSurface(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[
        ...
        if(budget?['canApprove']==true)Row(children:[Expanded(child:OutlinedButton(onPressed:_busy?null:()=>_status('reject'),child:const Text('No aprobar'))),const SizedBox(width:8),Expanded(child:FilledButton(onPressed:_busy?null:()=>_status('approve'),child:const Text('Aprobar')))]),
      ])),
